...
_readN:
	...
	xor ebx,ebx		; czyszczenie ebx
	mov ecx, N		
	mov edx, 10		; dlugosc znaków
	int 80h
	ret
	
_printNumber:	
	mov eax, [tempNumber]	; cmp tempNumber i tempN
	cmp eax, [tempN]
	jg _nextValue		; if tempNumber > tempN call _nextValue
	call _clearBuffer	;  clear bufer variable
	mov esi, bufer
	mov eax, [tempNumber]
	call int_to_string
	mov ecx, eax
	...
	mov eax, [tempNumber]		; set tempNumber to 1
	inc eax
	mov [tempNumber], eax
...
.next_digit:
  movzx eax,byte[esi]		; wrzucenie na eax kolejnego bajta od prawej z esi
  inc esi
  cmp eax, '0'			; sprawdzenie czy 0 <= eax <= 9
  jl .done
  cmp eax, '9'
  jg .done
...
.nextClear:
	mov byte[esi], 0
	inc esi
	loop .nextClear			; while ecx > 0 call .nextClear
	ret				; if ecx == 0
